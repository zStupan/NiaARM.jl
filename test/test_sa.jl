@testset "Simulated Annealing Tests" begin
    ...
    f5 = sa(sphere, problem, StoppingCriterion(maxevals=500), initial_temp=50.0, cooling_rate=0.98, seed=1234)
    f6 = sa(sphere, problem, StoppingCriterion(maxevals=500), step_size=0.2, seed=1234)

    @test_throws DomainError sa(sphere, problem, StoppingCriterion(maxiters=1000), initial_temp=-1.0)
    @test_throws DomainError sa(sphere, problem, StoppingCriterion(maxiters=1000), cooling_rate=1.5)
    @test_throws DomainError sa(sphere, problem, StoppingCriterion(maxiters=1000), step_size=-0.1)
    @test f3 <= 1e-5
    ...
    @test f5 == sa(sphere, problem, StoppingCriterion(maxevals=500), initial_temp=50.0, cooling_rate=0.98, seed=1234)
    @test f6 == sa(sphere, problem, StoppingCriterion(maxevals=500), step_size=0.2, seed=1234)
end
