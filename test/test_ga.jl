@testset "Genetic Algorithm Tests" begin
    ...
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), popsize=1)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), tournament_size=0)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), tournament_size=51)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), crossover_rate=-0.1)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), crossover_rate=1.1)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), mutation_rate=-0.1)
    @test_throws DomainError ga(sphere, problem, StoppingCriterion(maxiters=1000), mutation_rate=1.1)
    @test f3 <= 1e-5
